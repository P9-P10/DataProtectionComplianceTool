...
using GraphManipulation.Logging.LogSegmentation;
using Xunit;
...
public abstract class LogTest : IDisposable
{
    ...
    {
        var projectFolder = GetTestProjectFolder();
        return Path.Combine(projectFolder, "TestResources/logs");
    }
    ...
    protected static ConfigManager CreateConfigManager()
    {
        ...
        configManager.UpdateValue("LogPath", GetTestLogsFolderPath());
...
public class PlaintextLoggerTest : LogTest
{
    ...
    public void LoggerThrowsExceptionIfFilePathToLogNotAvailable()
    {
        ...
        configManager.UpdateValue("LogFolderPath", "");

        Assert.Throws<LoggerException>(() => new PlaintextLogger(configManager, new NeverSegmenter()));
    }
    ...
    //         logger.Append(log);
    //
    //         Assert.True(File.Exists(GetTestLogsFolderPath()));
    //     }
    //
    //     [Fact]
    ...
    //         logger.Append(log2);
    //
    //         var actual = File.ReadLines(GetTestLogsFolderPath()).ToList();
    //
    ...
    //
    //         File.WriteAllLines(GetTestLogsFolderPath(), new[] { log1String, log2String, log3String });
    //
    //         var configManager = CreateConfigManager();
    //         var logger = new PlaintextLogger(configManager);
    //
    ...
    //         }).ToList();
    //
    //         File.WriteAllLines(GetTestLogsFolderPath(), modifiedStrings);
    //
