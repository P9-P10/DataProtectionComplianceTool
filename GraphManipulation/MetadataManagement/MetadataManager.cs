...
public class MetadataManager : IMetadataManager
{
    ...
    public void MarkAsPersonalData(GDPRMetadata metadata)
    {
        ...
        var insertMetadataStatement = builder.Build() + " select last_insert_rowid();";
        var metadataId = _connection.QuerySingle<int>(insertMetadataStatement);

        var addReferencesToMetadataStatement = $@"
    ...
    {
        var valueUpdates = CreateValuesUpdates(value);

        var setValues = string.Join(", ", valueUpdates);
        var updateStatement = $@"
    ...
    {
        var columns = GetPropertiesWithAttributes<GDPRMetadata, ColumnAttribute>()
            .Select(pair => pair.Attribute.Name);

        var result = _connection.Query<GDPRMetadata>(@$"
