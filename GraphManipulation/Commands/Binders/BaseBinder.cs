...
public abstract class BaseBinder<TKey, TValue> : BinderBase<TValue> where TValue : Entity<TKey>, new()
{
        ...
        where TV : Entity<TK> where TK : notnull
    {
        ...
        catch (BindingException)
        {
            ...
            CommandHandler<TK, TV>.CreateHandler(key, manager, new FeedbackEmitter<TK, TV>(),
        ...
        where TV : Entity<TK>
    {
        ...
        {
            Console.Write(
                $"{key} {TypeToString.GetEntityType(typeof(TV))} does not exist. Would you like to create one? (y/n){Environment.NewLine}{CommandLineInterface.Prompt} ");
